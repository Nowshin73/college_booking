...
const UserDashboard = () => {
  return (
    <div className="min-h-screen font-serif bg-gradient-to-br from-blue-50 to-purple-100 p-6">

      {/* Page Title */}
      <div className="mb-8 bg-white/80 backdrop-blur rounded-xl p-6 shadow-md flex flex-col justify-center items-start">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {user.applied_colleges.map((college) => (
            <div key={college.college_id} className="bg-white rounded-lg shadow-md hover:shadow-xl transition p-4 border-l-4 border-indigo-500">
              ...
              <div className="mt-3 space-y-5 font-serif">
                <h3 className="text-2xl font-bold text-gray-800">{college.college_name}</h3>
                <p className="text-lg text-gray-500">Admission Date: {college.exam_date}</p>
                ...
                {!college.fee_paid && (
                  <button className="w-full mt-2 bg-indigo-600 hover:bg-indigo-700 text-white text-lg font-medium py-2 rounded-lg shadow">
                    💳 Pay Application Fee
                  </button>
                )}
              </div>
          ...
          {user.applied_colleges
            ...
            .map((college) => (
              <div key={college.college_id} className="flex justify-between items-center bg-white p-4 rounded-lg shadow-md border-l-4 border-purple-500">
                <div>
                  <p className="font-semibold md:text-xl mb-4 text-gray-800">{college.college_name}</p>
                  <p className="text-sm md:text-base text-gray-500">Exam Date: {college.exam_date}</p>
                </div>
      ...
      <section className="bg-gradient-to-r from-yellow-100 to-orange-100 shadow rounded-xl p-6 mb-10">
        ...
        <div className="space-y-3">
          {user.applied_colleges.map((college) => (
            <div key={college.college_id} className="p-4 bg-white rounded-lg shadow-md flex justify-between items-center border-l-4 border-yellow-500">
              <div>
                <p className="font-semibold md:text-xl mb-4 text-gray-800">{college.college_name}</p>
                <p className="text-sm md:text-base mb-4 text-gray-600">
                  Your Rank: {college.rank} / Called Students: {college.called_students}
                </p>
              </div>
      ...
      {/* Submit Research */}
      <section className="bg-gradient-to-r w-[700px] h-[300px] flex flex-col items-start justify-evenly from-blue-100 to-indigo-100 shadow rounded-xl p-6">
